class StudentCopy < Prawn::Document
    def initialize(students, gc_date)
        super(:page_size => "A4", :page_layout=>:landscape, background: open("app/assets/images/logo.png") )
        @students = students
        ...
        move_down 10
         ...
         ["Date of Admission: #{stud.admission_date}", "Traning Level: #{stud.study_level=="undergraduate"? "Bachelor Degree": "Masters Degree"}", "Date of Graduation: #{gc_date.strftime("%B,%d %Y")}"]], width: 700, :cell_style => { :border_width => 0, padding: 2, font_style: :bold, size: 11 }
        move_down 10
        ...
        student_copy.each do |key , value|
         ...
         move_down 10
         table each_stud_in_table(value), :cell_style=> {size: 8} do 
    ...
    def each_stud_in_table(value)
        student_grade = GradeReport.where(semester: value.first.semester, year: value.first.year).last
        [
        ...
        end+[[" ","","","",""," "],["SGPA", student_grade.sgpa, student_grade.total_credit_hour, "", student_grade.total_grade_point, ""],[" "," ","","",""," "]]
     end
    def header_footer
        # repeat :all do
