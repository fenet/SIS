ActiveAdmin.register ProgramExemption do
  ...
  batch_action "Approve application status by registrar", method: :put, confirm: "Are you sure?", if: proc { current_admin_user.role == "registrar head" } do |ids|
  ...
  batch_action "Reject application status by registrar", method: :put, confirm: "Are you sure?", if: proc { current_admin_user.role == "registrar head" } do |ids|
  ...
  batch_action "Approve application status by dean", method: :put, confirm: "Are you sure?", if: proc { current_admin_user.role == "dean" } do |ids|
  ...
  batch_action "Reject application status by dean", method: :put, confirm: "Are you sure?", if: proc { current_admin_user.role == "dean" } do |ids|
